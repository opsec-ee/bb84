 ...
 *
 * Four sidecar threads, four barriers:
 *
 *   FRONT --[fl]--> LEAD --[lr]--> REAR(QBER) --[rq]--> RECONCILE --[rc]--> REAR(PA)
 *
 *   barrier_fl  count=2  FRONT + LEAD
 *   barrier_lr  count=3  LEAD + REAR + RECONCILE
 *   barrier_rq  count=2  REAR(QBER done) + RECONCILE
 *   barrier_rc  count=2  RECONCILE done + REAR(PA)
 *
...
#define BENCH_RUNS 8  /* 8 runs -- sufficient for min/max/mean/spread at session timescale */
 ...
 *
 * FRONT: (store*, elapsed*) -- zeroed RAMStore + ee_ratio_t sink (AS)
 * LEAD:  four pthread_create + four pthread_join across four barriers (Pivot)
 ...
 *
 * Contract: {{0 [ (store*,elapsed*) (AS/--\WAS) GateResult ] 1}}
 ==================================================================
 */
[[nodiscard]]
...
int main(void)
{
    ...
    printf("Classical simulation of the BB84 quantum key distribution protocol\n");
    printf("with complete post-processing: error reconciliation via Cascade,\n");
    printf("information-theoretic privacy amplification, and key confirmation.\n");
    printf("No floats. No IEEE 754. OS-enforced memory immutability.\n");
    printf("Full ExCLisp AS/Pivot/IS contracts on every function.\n");
    printf("==========================================\n");

    /*
     * Self-test before any external interaction.
