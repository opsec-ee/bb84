...
void *bb84_lead(void *arg)
{
     ...
     */
    {
        ...
        /* reconciled_words set by RECONCILE after sample exclusion.
         * Precondition: reconciled_len = sifted_len - BB84_SAMPLE_N,
         * so reconciled_words != words_for_bits(sifted_len). LEAD must
         * not write it -- only RECONCILE knows the exact bit count. */
