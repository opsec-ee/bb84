 ...
 *
 * This thread entry contains two distinct Möbius crossings separated
 * by barrier_rq. Each phase has its own FRONT/LEAD/REAR contract:
 *
 * PHASE 1 contract (barrier_lr -> barrier_rq):
 *   FRONT: sifted_key PROT_READ -- BB84_N_PHOTONS raw gate states (AS)
 *   LEAD:  errors*RATIO_DENOM <= QBER_THRESH_N*sample -- cross-multiply
 *          comparison, no division (Pivot)
 *   REAR:  store->qber_e_num, store->sample_sift_idx[] committed (IS)
 *     X: upstream abort, alloc failure, Fisher-Yates failure, QBER exceeded
 *     1: QBER within threshold; sample positions written; RECONCILE unblocks
 *
 * PHASE 2 contract (barrier_rc -> return):
 *   FRONT: reconciled_key PROT_READ -- Cascade-corrected packed bits (AS)
 *   LEAD:  Toeplitz GF(2) matrix multiply: output[j]=XOR_i{r[j+i]&input[i]}
 *          (Pivot)
 *   REAR:  final_key PROT_READ sealed; confirm_hash set; session_gate (IS)
 *     X: h(e)>=0.5, PA length<=0, alice_confirm mismatch, alloc/seal failure
 *     1: final_key sealed, session_gate=GR_1, invariant held
 *
 * Thread contract (both phases combined):
 ...
 *
 * FRONT: (key, len, seed) -- final_key bytes, count, getrandom seed (AS)
 * LEAD:  h ^= key[i] << ((i%8)*8); h = (h<<7)|(h>>57) per byte (Pivot)
 * REAR:  uint64_t hash -- 64-bit fingerprint of key (IS)
 *   1: always (pure function, no failure path)
 *
 * Seed: getrandom-drawn uint64_t per session (see call site in bb84_rear).
 *   A fixed seed is a security risk -- an observer who knows the seed
 *   and sees the hash gains information about the key.
 *   A fresh per-session seed makes the hash output unpredictable to
 *   any attacker who does not know the seed.
 *   Seed is NOT stored in RAMStore -- it exists only in the call frame.
 *
 * Leaks 64 bits (hash) + 64 bits (seed if revealed) -- fully charged
 * to SECURITY_PARAM = 64. Seed must not be transmitted publicly.
 * Contract: {{0 [ (uint8_t*,size_t,uint64_t) (AS/--\WAS) uint64_t ] 1}}
 *            Lossy: 64-bit hash; collision possible by pigeonhole.
 ==================================================================
 */
static uint64_t confirm_hash_fn(const uint8_t *key, size_t len,
                                uint64_t seed)
{
    uint64_t h = seed; /* getrandom seed -- fresh per session, not fixed */
 ...
 *
 * Contract: {{0 [ RAMStore* (AS/--\WAS) bool ] 1}}
 *            Lossy: full key material collapsed to bool.
 ==================================================================
 */
[[nodiscard]]
...
void *bb84_rear(void *arg)
{
    ...
    *ret = GR_Z("rear not yet run");
     ...
     */
    {
        ...
        /* Draw fresh per-session seed for confirm_hash.
         * Fixed seeds are a security risk -- attacker who knows the
         * seed and observes the hash gains key information.
         * Seed lives only in this call frame; not stored anywhere. */
        uint64_t   hash_seed;
        GateResult hr_seed = rng_u64(&hash_seed);
        if (!GR_VALID(hr_seed)) {
            /* pa_out already freed above -- do not touch it */
            *ret = GR_X("rear p2: confirm_hash seed rng failed");
            return ret;
        }
        store->confirm_hash = confirm_hash_fn(store->final_key,
                                              final_bytes, hash_seed);
        memset(&hash_seed, 0, sizeof hash_seed); /* wipe seed from stack */
