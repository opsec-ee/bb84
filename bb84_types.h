 ...
 *  Max:        0.0005 s
 *  Mean:       0.0003 s
 *  Spread:     +-52% (Cascade correction count, not scheduler noise)
 *  Throughput: ~3,200,000-3,700,000 photons/s
 *
 *  Spread note: session completes in 0.2-0.5ms. Variance driven by
 *  Cascade correction count -- more channel errors = more bisect calls
 *  = more time. This is algorithmic variance, not scheduler noise.
 *  Gate-X rate ~5-13% at 3% noise (Binomial(256,0.03) tail). Correct.
 *
 ...
 *
 * Every GATE_X in this codebase is a projection of this invariant:
 *   QBER exceeded  -> Eve's information is not bounded  -> abort
 *   PA length <= 0 -> no secure bits extractable         -> abort
 *   Keys mismatch  -> Cascade failed; PA input corrupt   -> abort
 *   Sift too short -> insufficient entropy to sample      -> abort
 *
 * The five IFP phases below are the causal chain that enforces it.
 * Sealing order (mprotect PROT_READ) is the structural proof that
 * each phase's output is immutable before the next phase reads it.
 *
 *  I1: quantum_tx(n)     -> alice_raw[]      FRONT: Feistel-randomized
 *                                            basis + bit pairs,
 *                                            written once, sealed.
 *  I2: sift(alice_raw)   -> sifted_key[]     LEAD: basis match test;
 *                                            GATE_0/1 on match,
 *                                            GATE_X on mismatch.
 *  I3: reconcile(sifted) -> reconciled_key[] RECONCILE: Cascade-lite
 *                                            4-pass error correction;
 *                                            parity leakage tracked.
 *  I4: qber(sample)      -> RatioQBER        REAR p1: random sample,
 *                                            no prefix bias; integer
 *                                            arithmetic, no floats.
 *  I5: pa(n,e,leak)      -> final_key[]      REAR p2: Toeplitz hash;
 *                                            length = n*(1-2*h(e)) - leak
 *                                            - SECURITY_PARAM; h(e) is a
 *                                            sub-operation (table lookup,
 *                                            lossy projection, not a phase).
 *
 * =================================================================
 ...
 * SESSION PARAMETERS
 ==================================================================
 */
#define BB84_N_PHOTONS   ((size_t)2048u)  /* raw photons transmitted  */
#define BB84_SAMPLE_N    ((size_t)256u)   /* bits revealed for QBER   */
#define CASCADE_PASSES   ((uint32_t)4u)   /* Cascade-lite pass count  */
#define CASCADE_K0       ((uint32_t)8u)   /* block size pass 1        */
#define SECURITY_PARAM   ((uint32_t)64u)  /* bits subtracted post-PA  */
 ...
 *
 * 144000 = 2^7 * 3^2 * 5^3  (96 divisors: (7+1)(2+1)(3+1))
 ...
 * Security param : 64 bits subtracted post-PA unconditionally
 ==================================================================
 */
#define RATIO_DENOM      (144000ULL)
#define QBER_THRESH_N    (15840ULL)        /* 11%  */

#ifndef NOISE_RATE_N
#define NOISE_RATE_N     (4320ULL)         /*  3% default */
#endif
...
#define HE_TABLE_LEN     ((size_t)12u)
 ...
 *
 * Contract: {{0 [ e_num:u64 (AS/--\WAS) h_num:u64 ] 1}}
