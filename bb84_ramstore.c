 ...
 * Contract: {{0 [ size_t (AS/--\WAS) size_t ] 1}}
 *            Lossy: all values in the same page bucket map to one output.
 */
 ...
 *
 * Contract: {{0 [ size_t (AS/++\PLUS) void* ] 1}}
 *            Expanding: produces new memory from the kernel.
 */
 ...
 *   1: slab sealed
 *
 * Contract: {{0 [ (void*,size_t) (AS/--\WAS) GateResult ] 1}}
 *            Lossy: mprotect syscall collapses slab state to pass/fail.
 */
 ...
 *   1: sealed
 *
 * Contract: {{0 [ RAMStore* (AS/--\WAS) alice_raw sealed ] 1}}
 */
 ...
 *
 * Contract: {{0 [ RAMStore* (AS/--\WAS) sifted_key sealed ] 1}}
 */
 ...
 *   1: sealed
 *
 * Contract: {{0 [ RAMStore* (AS/--\WAS) reconciled_key sealed ] 1}}
 */
 ...
 *
 * Contract: {{0 [ RAMStore* (AS/--\WAS) final_key sealed ] 1}}
